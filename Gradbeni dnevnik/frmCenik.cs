...
namespace Komunala
{
    public partial class frmCenik : Form
    {
...
//        float tobcina = 0;
        ...
        private void Display()  // prikaži mrežo
        {
            ...
            if (prikazi_vse==true)
            {
                ...
                q = "select * from tbl_Cenik order by skupina, storitev"; //vse skupina + sort;
            }
            else
            {
                cb1.Text = skupina_za_prikaz_str;

                q = "select * from tbl_Cenik where skupina = @tmpskupina order by skupina, storitev"; //ena skupina + sort;
            }
            Izprazni_dgv();
            try
            {
                ...
                while (rdr.Read())
                {
                    ...
                    dgv2.Rows.Add("", "", "Cene za Občino", "Cene za trg");
                    dgv2.Columns[0].Width = 359;
                    dgv2.Columns[1].Width = sirina_znesek*3;
                    ...
                    kolobcina22.DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;
        ...
        private void Brisi()
        {
            ...
            if (result == DialogResult.Yes)
            {
                try
                {
                    q = "delete from tbl_cenik where id = @tid";
                    cmd = new SqlCommand(q, con);
                    con.Open();
            ...
            if (ne_shrani==0)
            {
                ...
                try
                {
                    ...
                    if (dodajanje == true)
                    {
                        ...
                        // dodaj
                        q = "insert into tbl_Cenik(storitev,em,cena_trg,cena_obcina,skupina) values(@tstoritev,@tem,@tcenat,@tcenao,@tskupina)";
                            cmd = new SqlCommand(q, con);
                            con.Open();
                    ...
                    if (spreminjanje == true)
                      ...
                      //  else
                       ...
                       // MessageBox.Show("Shranjujem spreminjanje!");
                        q = "update tbl_Cenik set storitev=@tstoritev, em = @tem, cena_trg= @tcenat, cena_obcina=@tcenao, skupina=@tskupina where id=@tid";
                            cmd = new SqlCommand(q, con);
                            con.Open();
                            cmd.Parameters.AddWithValue("@tstoritev", id_storitev);
                            cmd.Parameters.AddWithValue("@tem", id_enota); 
                ...
                btnBrisi.Enabled = true;
                btnSpremeni.Enabled = true;
                btnIzpiso.Enabled = true;
                btnIzpist.Enabled = true;
                btnNazaj.Enabled = true;
                Display();    //?????
            }
        }
        ...
        private void Dodaj()
        {
            ...
            btnIzpiso.Enabled = false;
            btnIzpist.Enabled = false;
            cb2.Focus();
        }
        ...
        private void Spremeni()
        {
            ...
            if (row_count > 0)
            {
                ...
                q = "select * from tbl_Cenik where id = @tmpid"; //+ sort;
        ...
        private void tb4_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                double tmp_cena = Convert.ToDouble(tb4.Text);
                tb5.Text = Convert.ToString(Izračunaj_ddv22(tmp_cena));
                tb6.Text = Convert.ToString(Izračunaj_ddv95(tmp_cena));
                btnShrani.Focus();
            }
            if (e.KeyCode == Keys.Escape)
            {
        ...
        private void tb4_Leave(object sender, EventArgs e)
        {
            ...
            // zamenjaj decimalno piko z vejico
            string tmp_4 = tb4.Text;
            tmp_4 = tmp_4.Replace(".", ",");
            tb4.Text = tmp_4;

            if (IsNumeric(tb4.Text) == false)
            {
                MessageBox.Show("Cena mora biti število!");
                tb4.Focus();
            }
            else
            {
        ...
        private void btnIzpiso_Click(object sender, EventArgs e)
        {
            // izpis cenika za občino
        ...
        private void frmCenik_KeyDown(object sender, KeyEventArgs e)
        {
